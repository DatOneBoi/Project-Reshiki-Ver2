accept_key = keyboard_check_pressed(ord("E")) || mouse_check_button_pressed(mb_left);
...
if setup == false
	{
		...
		for(var p = 0; p < page_number; p++)
			...
			//get the x position for the textbox
				...
				//no character sprite (centered text box)
				if speaker_sprite[p] == noone{
				text_x_offset[p] = 30;
				}
				
			
			//setting individual characters and finding where the lines of texts should break
			for (var c = 0; c < text_length[p]; c++)
				...
				//geeting each characters coordinates
				for (var c = 0; c < text_length[p]; c++)
					{
...
if speaker_sprite[page] != noone
	{
		...
		//draw the speaker
		draw_sprite_ext(txtb_spr[page], txtb_img, textbox_x + portrait_x_offset[page] - 255, textbox_y, sprite_width/txtb_spr_w, sprite_height/txtb_spr_h, 0, c_white, 1);
		draw_sprite_ext(sprite_index, image_index, _speaker_x - 258, textbox_y, speaker_side[page], 1 ,0 ,c_white, 1);
		
	}

//back of the text box
draw_sprite_ext(txtb_spr[page], txtb_img, _txtb_x + 5, _txtb_y, textbox_width / txtb_spr_w, textbox_height/ txtb_spr_h, 0, c_white, 1 );
		...
		for (var op = 0; op <option_number; op++)
			{
				...
				draw_sprite_ext(txtb_spr[page], txtb_img, _txtb_x + 10, _txtb_y - 2 - _op_space * option_number + _op_space * op, _o_w/txtb_spr_w, (_op_space-1)/txtb_spr_h, 0, c_white, 1)
				
				//the arrow
				if option_pos == op
					{
						draw_sprite(spr_txt_arrow, 0 , _txtb_x - 9, _txtb_y - 2 - _op_space * option_number + _op_space * op );
					}
				
				//the option text
				draw_text(_txtb_x + 10 + _op_border,  _txtb_y - 2 - _op_space * option_number + _op_space * op + 2, option[op]);
