//draw background
draw_sprite(spr_BattleBG, 0, x, y);
...
//draw ui boxes
draw_sprite_stretched(spr_sbox, 0, x+75, y+155, 212, 60);
draw_sprite_stretched(spr_sbox, 0, x+1, y+155, 73, 60);
...
for (var i = 0; i < array_length(partyUnits); i++)
{
	...
	if (_char.hp <= 0) draw_set_color(c_red);
	draw_text(x+COLUMN_HP+22, y+170+(i*12),string(_char.hp) + "/" + string(_char.hpMax));
	
	draw_set_color(c_white);
	if (_char.mp < (_char.mpMax * 0.5)) draw_set_color(c_orange);
	if (_char.mp <= 0) draw_set_color(c_red);
	draw_text(x+COLUMN_MP+14, y+170+(i*12),string(_char.mp) + "/" + string(_char.mpMax));
...
if (cursor.active)
{
	with (cursor)
	{
		if (activeTarget != noone)
		{
			...
			{
				draw_sprite(spr_pointer, 0 , activeTarget.x-10, activeTarget.y);
			}
			else
			{
				...
				{
					draw_sprite(spr_pointer, 0, activeTarget[i].x-10, activeTarget[i].y);
				}
				draw_set_alpha(1.0);
			}
		}
...
if (battleText !="")
{
	...
	draw_sprite_stretched(spr_sbox, 0, x+160-(_w*0.5),y+5,_w,25);
