...
function ss_item_key_from_struct(itemStruct) {
	if (!is_struct(global.item_list)) return "";
	var names = variable_struct_get_names(global.item_list);
	var n = array_length(names);
	for (var i = 0; i < n; i++) {
		var key = names[i];
		if (global.item_list[? key] == itemStruct) return key;
	}
	return "";
}

function ss_inventory_keys_from_current() {
	var keys = array_create(0);
	if (instance_exists(obj_item_manager)) {
		var invLen = array_length(obj_item_manager.inv);
		for (var i = 0; i < invLen; i++) {
			var k = ss_item_key_from_struct(obj_item_manager.inv[i]);
			if (is_string(k) && k != "") {
				array_push(keys, k);
			}
		}
...
function ss_inventory_from_keys(keys) {
	var out = array_create(0);
	if (!is_array(keys) || !is_struct(global.item_list)) return out;
	var len = array_length(keys);
	for (var i = 0; i < len; i++) {
		var key = keys[i];
		if (is_string(key) && variable_struct_exists(global.item_list, key)) {
			array_push(out, global.item_list[? key]);
		}
	}
	return out;
}
...
function savegame_save(_filename) {
	...
	var state = {
		...
		player: {
			x: obj_guy.x,
			y: obj_guy.y,
			face: (instance_exists(obj_guy) ? obj_guy.face : 0),
		},
		globals: {
		...
		inventory: ss_inventory_keys_from_current(),
	};
...
function savegame_load(_filename) {
	...
	if (is_struct(parsed.player) && is_real(parsed.player.face)) {
		global.load_face = parsed.player.face;
	}
	...
	// Stash inventory keys to rebuild after room change
	global.ss_loaded_inventory_keys = parsed.inventory;
	...
	if (targetRoomIndex == room) {
		...
		// Rebuild inventory immediately if manager exists
		if (instance_exists(obj_item_manager) && is_array(parsed.inventory)) {
			obj_item_manager.inv = ss_inventory_from_keys(parsed.inventory);
		}
